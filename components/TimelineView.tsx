...
// ─── Status pill - technical design ────────────────────────────────────────────────
const STATUS_STYLES: Record<string, string> = {
  idle: "bg-semantic-error/5 text-semantic-error border-semantic-error/30",
  active:
    "bg-semantic-warning/5 text-semantic-warning border-semantic-warning/30",
  completed:
    "bg-semantic-success/5 text-semantic-success border-semantic-success/30",
};
...
function StatusPill({ status }: { status: string }) {
  return (
    <span
      className={cn(
        "text-[10px] px-1.5 py-0.5 font-mono font-bold uppercase tracking-wider border",
        STATUS_STYLES[status] ??
          "bg-text-muted/5 text-text-muted border-border-subtle",
      )}
    >
...
function TagChip({
  ...
  return c ? (
    <span
      className="text-[9px] px-1.5 py-0.5 font-mono font-bold uppercase tracking-wider border"
      style={{ backgroundColor: `${c}15`, color: c, borderColor: c }}
    >
      {name}
    </span>
  ) : (
    <span className="text-[9px] px-1.5 py-0.5 font-mono font-bold uppercase tracking-wider border border-border-subtle text-text-muted">
      {name}
    </span>
  );
}
...
function NoteCard({
  ...
  const overdue =
    !!note.dueDate && note.status !== "completed" && note.dueDate < Date.now();
  ...
  return (
    <motion.article
      ...
      onClick={() => onEdit(note)}
      className="group relative cursor-pointer border border-border-subtle bg-bg-elevated hover:border-border-default transition-all duration-200 overflow-hidden"
    >
      {/* Type accent line */}
      <div className="absolute left-0 inset-y-0 w-[2px] bg-border-subtle group-hover:bg-[#a855f7] transition-colors" />
      ...
      <div className="pl-5 pr-5 py-4">
        ...
        <div className="flex items-center gap-2 mb-2.5">
          ...
          <span className="text-[10px] font-mono font-bold tracking-wider uppercase text-text-muted/60">
            NOTE
          </span>
        ...
        {/* Title */}
        <p className="text-sm font-mono font-semibold text-text-primary group-hover:text-accent-primary transition-all duration-200 leading-snug mb-1.5">
          {note.title}
        </p>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {note.tags?.map((t) => (
            <TagChip key={t} name={t} globalTagColors={globalTagColors} />
          ))}
          {note.dueDate && (
            <span
              className={cn(
...
function TaskCard({
  ...
  const overdue =
    !!task.dueDate && task.status !== "completed" && task.dueDate < Date.now();
  ...
  return (
    ...
    >
      {/* Type accent line */}
      <div className="absolute left-0 inset-y-0 w-[2px] bg-border-subtle group-hover:bg-[#0ea5e9] transition-colors" />
      ...
      <div className="pl-5 pr-5 py-4">
        ...
        {/* Title */}
        <p className="text-sm font-mono font-semibold text-text-primary group-hover:text-accent-secondary transition-all duration-200 leading-snug mb-1.5">
          {task.title}
        </p>
...
function SectionHeader({ label, count }: { label: string; count: number }) {
  return (
    <div className="flex items-center gap-3 py-1">
      <span className="text-xs font-mono font-bold text-text-muted/60 uppercase tracking-wider shrink-0">
        {label}
      </span>
      <div className="flex-1 h-px bg-border-subtle" />
      <span className="text-[10px] font-mono text-text-muted/40 tabular-nums shrink-0">
        {count}
      </span>
    </div>
  );
...
export function TimelineView({
  ...
  const buckets = useMemo(() => {
    ...
    const all: AnyItem[] = [...noteItems, ...taskItems].sort(
      ...
      <div className="px-4 py-8 space-y-6">
        {[0, 1, 2].map((i) => (
          <div key={i} className="space-y-3">
            <div className="h-3 bg-border-subtle/40 animate-pulse w-24" />
            {[0, 1].map((j) => (
              <div
                key={j}
                className="h-24 bg-bg-elevated border border-border-subtle animate-pulse"
              />
            ))}
          </div>
        ))}
  ...
  if (buckets.length === 0) {
    return (
      <div className="py-20 flex flex-col items-center gap-4 text-center">
        <div className="w-12 h-12 border border-border-subtle bg-bg-elevated flex items-center justify-center">
          <svg
