...
function MarkdownPreview({ content }: { content: string }) {
  if (!content)
    return (
      <p className="text-text-muted text-sm italic p-4">
  ...
  return (
    <div className="p-5 overflow-y-auto h-full text-sm leading-relaxed">
      <ReactMarkdown
        ...
        components={{
          h1: ({ children }) => (
            <h1 className="text-xl font-bold text-text-primary mt-4 mb-2 pb-1 border-b border-border-subtle first:mt-0">
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-lg font-semibold text-text-primary mt-4 mb-1.5 first:mt-0">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
          ...
          p: ({ children }) => (
            <p className="text-text-primary mb-3 leading-relaxed last:mb-0">
              {children}
            </p>
          ),
          strong: ({ children }) => (
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const publicUrl = await getImageUrl({
        storageId: storageId as Parameters<typeof getImageUrl>[0]["storageId"],
      });
      if (publicUrl) {
        ...
        <TB title="Numbered list" onClick={() => insert("1. ", "", "item")}>
          ...
          >
            <path
              ...
              strokeLinejoin="round"
              strokeWidth={2}
              d="M8 6h12M8 12h12M8 18h12M4 5v2M3 11h2M3 17h2"
            />
          </svg>
        </TB>
      ...
      <div className="flex flex-1 min-h-[500px] overflow-hidden">
        {(mode === "edit" || mode === "split") && (
          <textarea
            ...
            rows={3}
            className={cn(
              "resize-none outline-none bg-transparent text-sm text-text-primary placeholder:text-text-muted p-5 font-mono leading-relaxed",
              mode === "split"
